//
//  Copyright © 2017 Bosma. All rights reserved.
//
...
#include <croncpp/croncpp.hpp>
#include <ctpl/ctpl_stl.hpp>
...
class CronTask : public Task {
 public:
    CronTask(std::string expression, std::function<void()> &&f)
        : Task(std::move(f), true), cron(cron::make_cron(expression)) {}
